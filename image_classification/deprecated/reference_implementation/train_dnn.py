...
def run(rank, size):
    ...
    tmp_clifreq = [torch.tensor(0, dtype=torch.float32) for _ in range(dist.get_world_size())]

    print('about to select clients')
    dist.barrier()
    # select client for each round, in total m ranks
    send = torch.zeros(args.size, dtype=torch.float32)
    if rank == 0:
        replace_param = False
        ...
        send = torch.tensor(idxs_users, dtype=torch.float32)
    dist.barrier()
    ...
    for rnd in range(args.rounds):
        ...
        dist.all_gather(tmp_cli, torch.tensor(loss_final, dtype=torch.float32))
        dist.all_gather(tmp_clifreq, torch.tensor(sel_idx, dtype=torch.float32))
        dist.barrier()
        for i, i_val in enumerate(tmp_clifreq):
            cli_freq[int(i_val.item())]+= 1         # Cli freq is the entire clients that are selected for all rounds
            cli_val[int(i_val.item())] = tmp_cli[i].item()
        ...
        if rank == 0:
            ...
            print('rank:', rank, 'test_acc:', test_acc, 'test_loss:', test_loss)
            ...
            print('rank:', rank, 'train_loss:', train_loss, 'train_loss1:', train_loss1)
        dist.barrier()
        ...
        dist.broadcast(tensor=send, src=0)
        dist.barrier()
        print(f'next round rank: {rank}, send: {send}')
        sel_idx = int(send[rank])
